...
def main(cfg: DictConfig) -> None:
    ...
    try:
        ...
        task = Task.init(project_name="BESS-JEPA", task_name="pretrain_hybrid_projection_jepa")
        task.connect(OmegaConf.to_container(cfg, resolve=True))
    except Exception as e:
        print(f"ClearML unavailable: {e}")

    dm = HybridPretrainLoader(**cfg.data)
    model = HybridProjectionJEPA(**cfg.model)

    trainer = L.Trainer(
        max_epochs=cfg.trainer.max_epochs,
        accelerator=cfg.trainer.accelerator,
        devices=cfg.trainer.devices,
        logger=TensorBoardLogger(save_dir="logs/pretrain", name="hybrid_projection_jepa"),
        callbacks=[
            EarlyStopping(monitor="val/loss", patience=cfg.trainer.patience, mode="min"),
            ModelCheckpoint(monitor="val/loss", mode="min", save_top_k=1, filename="best-hybrid-projection-jepa"),
        ],
    )
