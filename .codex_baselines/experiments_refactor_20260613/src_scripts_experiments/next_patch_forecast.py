...
def evaluate_rollout(
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(loader):
            ...
            eval_steps = steps or available_steps
            target = target[:, :eval_steps]

            pred = rollout_next_patch(model, x, eval_steps)
            err = pred - target
...
def plot_examples(pred: np.ndarray, target: np.ndarray):
    import matplotlib
...
def main(cfg: DictConfig) -> None:
    ...
    fig = plot_examples(pred, target)
    report_figure(task, "Next Patch Forecast", "examples", fig, report_dir / "next_patch_examples.png")
    ...
    (report_dir / "next_patch_rollout_metrics.json").write_text(json.dumps(rollout_metrics, indent=2))
    ...
    report_figure(task, "Next Patch Forecast", "rollout_example", fig, report_dir / "next_patch_rollout.png")
    ...
    report_figure(
        ...
        "cumulative_mse",
        fig,
        report_dir / "next_patch_cumulative_mse.png",
    )
    ...
    report_figure(
        ...
        "step_mse_band",
        fig,
        report_dir / "next_patch_step_mse_band.png",
    )
    ...
    report_figure(
        task,
        "Next Patch Forecast",
        "channel_step_mse_bands",
        fig,
        report_dir / "next_patch_channel_step_mse_bands.png",
    )
    ...
    report_figure(
        ...
        "sample_count",
        fig,
        report_dir / "next_patch_sample_count.png",
    )

    final_summary = make_final_summary(test_results, rollout_metrics, rollout_steps)
    report_summary(task, final_summary)
    (report_dir / "next_patch_final_summary.json").write_text(json.dumps(final_summary, indent=2))
